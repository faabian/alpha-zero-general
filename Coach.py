...
class Coach:
    ...
    def __init__(self, game, nnet, args):
        ...
        self.traces = []  # MCTS search traces to dump
    ...
    def save_selfplay(self):
        """
        Run self-play and save game episodes without training. Do not use in conjunction with `learn`.
        """
        for i in range(1, self.args.numIters + 1):
            # bookkeeping
            log.info(f"Starting Iter #{i} ...")
            ...
            # save current iteration's episodes
    ...
    def save_traces(self, iteration):
        output_path = Path(self.args.checkpoint) / f"traces_{iteration:03}.jsonl"
        with open(output_path, "w") as f:
            f.write("\n".join(json.dumps(d) for d in self.traces) + "\n")
